...
#import "WildIMKitDatabasePool.h"
    ...
    #define WildDBAutorelease(__v) ([__v autorelease]);
    #define WildDBReturnAutoreleased WildDBAutorelease

    #define WildDBRetain(__v) ([__v retain]);
    #define WildDBReturnRetained WildDBRetain

    #define WildDBRelease(__v) ([__v release]);

    #define WildDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define WildDBAutorelease(__v)
    #define WildDBReturnAutoreleased(__v) (__v)

    #define WildDBRetain(__v)
    #define WildDBReturnRetained(__v) (__v)

    #define WildDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define WildDBDispatchQueueRelease(__v)
    #else
        #define WildDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^WildDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);


/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in WildDB are:

 - `WildIMKitDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<WildIMKitResultSet>` - Represents the results of executing a query on an `WildIMKitDatabase`.
 - `<WildIMKitDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<WildIMKitDatabasePool>` - A pool of `WildIMKitDatabase` objects.
 - `<WildIMKitStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [WildDB on GitHub](https://github.com/ccgus/wilddb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [WildDB mailing list](http://groups.google.com/group/wilddb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `WildIMKitDatabase` object and use it across multiple threads. Instead, use `<WildIMKitDatabaseQueue>`.
...
@interface WildIMKitDatabase : NSObject  {
...
/** Create a `WildIMKitDatabase` object.
 
 An `WildIMKitDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `WildIMKitDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `WildIMKitDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    WildIMKitDatabase *db = [WildIMKitDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    WildIMKitDatabase *db     = [WildIMKitDatabase databaseWithPath:dbPath];
 ...
 @return `WildIMKitDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `WildIMKitDatabase` object.
...
/** Execute single update statement
 ...
 @note If you want to use this from Swift, please note that you must include `WildIMKitDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeUpdate:withArgumentsInArray:>`.
 */
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(WildDBExecuteStatementsCallbackBlock)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<WildIMKitResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[WildIMKitResultSet next]>`) from one record to the other.
 ...
 @return A `<WildIMKitResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see WildIMKitResultSet
 @see [`WildIMKitResultSet next`](<[WildIMKitResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 
 @note If you want to use this from Swift, please note that you must include `WildIMKitDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeQuery:withArgumentsInArray:>`.
 */

- (WildIMKitResultSet *)executeQuery:(NSString*)sql, ...;
...
- (WildIMKitResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (WildIMKitResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (WildIMKitResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (WildIMKitResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
+ (NSString*)WildDBUserVersion;

+ (SInt32)WildDBVersion;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(WildIMKitDatabase *adb) {
        ...
        int rowCount = 0;
        WildIMKitResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
            NSLog(@"Does %@ start with 'h'?", [rs stringForColumnIndex:0]);
        }
        WildDBQuickCheck(rowCount == 2);
    }];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [WildIMKitDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one WildDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using WildIMKitDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to WildDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using WildDB you will not need to interact directly with `WildIMKitStatement`, but rather with `<WildIMKitDatabase>` and `<WildIMKitResultSet>` only.
...
@interface WildIMKitStatement : NSObject {
