...
@interface WildIMKitDatabasePool()

- (void)pushDatabaseBackInPool:(WildIMKitDatabase*)db;
- (WildIMKitDatabase*)db;

@end


@implementation WildIMKitDatabasePool
...
+ (instancetype)databasePoolWithPath:(NSString*)aPath flags:(int)openFlags {
    return WildDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags {
    ...
    if (self != nil) {
        _path               = [aPath copy];
        _lockQueue          = dispatch_queue_create([[NSString stringWithFormat:@"wilddb.%@", self] UTF8String], NULL);
        _databaseInPool     = WildDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = WildDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    WildDBRelease(_path);
    WildDBRelease(_databaseInPool);
    WildDBRelease(_databaseOutPool);
    
    if (_lockQueue) {
        WildDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(WildIMKitDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The WildIMKitDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (WildIMKitDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [WildIMKitDatabase databaseWithPath:self->_path];
