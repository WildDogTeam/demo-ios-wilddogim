...
+ (instancetype)resultSetWithStatement:(WildIMKitStatement *)statement usingParentDatabase:(WildIMKitDatabase*)aDB {
    ...
    return WildDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    WildDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return WildDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"WildIMKitDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
...
- (void)setParentDB:(WildIMKitDatabase *)newDb {
    _parentDB = newDb;
}
